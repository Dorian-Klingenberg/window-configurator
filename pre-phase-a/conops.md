...
- **Blazor / C# only — no JavaScript:** The production-adjacent implementation is Blazor-based. The existing Knockout.js + Bootstrap 3 frontend is the migration target. The C# pricing engine (already implemented server-side) runs in the browser via Blazor WebAssembly, satisfying N-10 without a separate JavaScript pricing engine.
